...
python_dir="$HOME/git/scripts/python"
...
| rofi -matching fuzzy -dmenu -i -mesg "How should I open '$visual'?" -select
  ...
  case "$link_handlers" in
    umpv) "$python_dir/umpv" "$url" & disown ;;
    umpv_audio) "$python_dir/umpv_audio" "$url" & disown ;;
    mpv) mpv "$url" 2>/dev/null & disown ;;
    bookmark) buku --add "$url" toread "$feed_title" --title "$title" ;;
    firefox) firefox "$url" ;;
    ...
    paragraph) /bin/rm /tmp/para.txt; "$python_dir/paragraphscraper/paragraph_scraper.py" --url "$url" && alacritty --hold -e bat /tmp/para.txt ;;
    summary) /bin/rm /tmp/para.txt; "$python_dir/paragraphscraper/paragraph_scraper.py" --url "$url" && "$python_dir/paragraphscraper/article_summarise_cosine.py" --file-name /tmp/para.txt --context > /tmp/summary.txt && alacritty --hold -e bat /tmp/summary.txt ;;
    ytdl) youtube-dl "$url" > /dev/null 2>&1 & ;;
    speedread) /bin/rm /tmp/para.txt; "$python_dir/paragraphscraper/paragraph_scraper.py" --url "$url" && alacrity --hold -e /usr/bin/speedread -w 350 /tmp/para.txt ;;
    speedread_summary) /bin/rm /tmp/para.txt; "$python_dir/paragraphscraper/paragraph_scraper.py" --url "$url" && "$python_dir/paragraphscraper/article_summarise_cosine.py" --file-name /tmp/para.txt --context > /tmp_summary.txt && alacritty --hold -e /usr/bin/speedread -w 350 /tmp/summary.txt ;;
  esac
fi
